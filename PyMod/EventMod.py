...
EVENT_DATAFEED='EventDataFeed'
EVENT_ORDERRETURN='EventOrderReturn'
...
class EventEngine(object):
    ...
    __deal: 私有方法，处理事件，调用注册在引擎中的监听函数
    __onTimer：私有方法，计时器固定事件间隔触发后，向事件队列中存入计时器事件
    Start: 公共方法，启动引擎
    Stop：公共方法，停止引擎
    Register：公共方法，向引擎中注册监听函数
    Unregister：公共方法，向引擎中注销监听函数
    AddEvent：公共方法，向事件队列中存入新的事件
...
def test():
    ...
    def simpletest(event):
        print('%s:正在处理第%s个事件' % (str(datetime.now()),event.Value['Num']))
    ...
    for i in range(1,20):
        ...
        TestEvent.Value['Num']=i
        ee.AddEvent(TestEvent)
        time.sleep(5)
    ee.Stop()
