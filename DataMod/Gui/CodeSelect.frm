...
'添加板块分类
Dim nodx As Node
Plate = CodeTran.ReadUTF8File(App.Path + "\Setting\Plate.ini")
...
Private Sub CB_getPlaeSymbol_Click()
    ...
    AimStr = UsedMod.MakeGetDataStr(, , AimFields, AimDataType, , AimDataSource, AimSpecialConfig)
    TempPath = App.Path + "\Cache\CodeSelect.gd"
    TempSavePath = App.Path + "\Cache\CodeSelect.gdret"
    ret = CodeTran.WriteUTF8File(TempSavePath + vbNewLine + AimStr + vbNewLine + "---", TempPath)
    ret = UsedMod.PyEx(GetData.PythonPath, GetData.PY_GetData_Path, TempPath)
    'UsedMod.Cmd3 ("E:\Miniconda3-Py3.5.2\python.exe E:/OneDrive/0_Coding/010_MyQuantSystem/Beta3.0/DataMod/GetData.py")
    '到这代表选股窗口选股成功-------------------------------------------------------
    LB_CodeList.Clear
    TempStr = CodeTran.ReadUTF8File(TempSavePath)
...
Private Sub CB_SureSelect_Click()
    RetCodeSelect = UsedMod.GetListBoxSelectedText(LB_CodeList)
...
'    RetCodeSelect = Replace(RetCodeSelect, "]", "")
    ...
    GetData.CODE_LIST.Clear
    For i = 0 To UBound(TempCodeSelected)
        GetData.CODE_LIST.AddItem (TempCodeSelected(i))
        GetData.CODE_LIST.Selected(i) = True
