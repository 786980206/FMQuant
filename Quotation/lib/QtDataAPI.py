...
#行情采样类型
EQuoteType = {"k_QuoteNull":QtPyAPI.EQuoteType.k_QuoteNull,
              "k_Second":QtPyAPI.EQuoteType.k_Second,#秒，日内高频
...
EQuoteData = {"k_QuoteNone":QtPyAPI.EQuoteData.k_QuoteNone,
              ...
              "k_Sts":QtPyAPI.EQuoteData.k_Sts,    #整点分时消息，沪深L1、中金所L2、商品期货L1、上海L1个股期权
              ...
              "k_StsPerTick":QtPyAPI.EQuoteData.k_StsPerTick, #Tick分时消息，沪深L1、中金所L2、商品期货L1、上海L1个股期权
              "k_L1PriceGreekLetter":QtPyAPI.EQuoteData.k_L1PriceGreekLetter, #L1欧式期权和希腊字母
              "k_PubMinLDIndex":QtPyAPI.EQuoteData.k_PubMinLDIndex, #领先指数品种
              "k_L2IndexRDEVolume":QtPyAPI.EQuoteData.k_L2IndexRDEVolume, #指数涨跌平家数
              "k_L2TenSecMinQuote":QtPyAPI.EQuoteData.k_L2TenSecMinQuote, #L2行情十秒分时
              "k_L2TenSecMinTran":QtPyAPI.EQuoteData.k_L2TenSecMinTran, #L2逐比成交十秒分时
             }

#月度计费数据
EBillDataType = {"k_BillAll":QtPyAPI.EBillDataType.k_BillAll,#不区分类型
                "k_BillQuote":QtPyAPI.EBillDataType.k_BillQuote,#行情数据
                "k_BillBasic":QtPyAPI.EBillDataType.k_BillBasic,#基本面数据
                "k_BillFactor":QtPyAPI.EBillDataType.k_BillFactor#基本面数据
              }

ECalcSign = {"k_EQ":QtPyAPI.ECalcSign.k_EQ,#符号为"= "
             "k_GT":QtPyAPI.ECalcSign.k_GT,#大于
             "k_GE":QtPyAPI.ECalcSign.k_GE,#大于等于
             "k_LT":QtPyAPI.ECalcSign.k_LT,#小于
             "k_LE":QtPyAPI.ECalcSign.k_LE,#小于等于
             "k_BT":QtPyAPI.ECalcSign.k_BT,#BetweenTo
             "k_IN":QtPyAPI.ECalcSign.k_IN,#包含于, IncludeIn
             "k_LK":QtPyAPI.ECalcSign.k_LK,#字符的Like
             "k_NE":QtPyAPI.ECalcSign.k_NE,#NoEqual
             "k_NI":QtPyAPI.ECalcSign.k_NI#NotIn
              }
...
def FreePyThread(quoteData):
    QtPyAPI.FreePyThread(quoteData);

# @brief 注册整点实时分时数据回调方法
# @param cbFunction 接收数据函数对象
def RegSubStsCB(cbFunction):
    return QtPyAPI.SetSubStsCB(cbFunction)

# @brief 注册tick实时分时数据回调方法
# @param cbFunction 接收数据函数对象
def RegSubStsPerTickCB(cbFunction):
    return QtPyAPI.SetSubStsPerTickCB(cbFunction)
...
# @brief 注册L1欧式期权和希腊字母
# @param cbFunction 接收数据函数对象
def RegSubL1PriceGreekLetterCB(cbFunction):
    QtPyAPI.SetSubL1PriceGreekLetterCB(cbFunction);

# @brief 注册领先指数品种
# @param cbFunction 接收数据函数对象
def RegSubPubMinLDIndexCB(cbFunction):
    QtPyAPI.SetSubPubMinLDIndexCB(cbFunction);

# @brief 注册指数涨跌平家数
# @param cbFunction 接收数据函数对象
def RegSubL2IndexRDEVolumeCB(cbFunction):
    QtPyAPI.SetSubL2IndexRDEVolumeCB(cbFunction);

# @brief 注册L2行情十秒分时
# @param cbFunction 接收数据函数对象
def RegSubL2TenSecMinQuoteCB(cbFunction):
    QtPyAPI.SetSubL2TenSecMinQuoteCB(cbFunction);

# @brief 注册L2逐比成交十秒分时
# @param cbFunction 接收数据函数对象
def RegSubL2TenSecMinTranCB(cbFunction):
    QtPyAPI.SetSubL2TenSecMinTranCB(cbFunction);

# @brief 注册连接状态回调方法
# @param cbFunction 回调函数
def RegStatusCB(cbFunction):
    QtPyAPI.SetStatusCB(cbFunction);
...
# @param reserve 预留，默认为空
def GetTradeCalendar(exchangeCode, dateBegin, dateEnd, fields, dayOfWeek = [], tradeType = [], \
                     open = EQtBool["k_QtNull"], timezone = 8, securityType = "", reserve = ""):
    ...
    ret = QtPyAPI.GetTradeCalendarForPy(cols, errorMsg, exchangeCodeTmp, dateBegin, dateEnd,\
                                        fieldsTmp, dayOfWeekTmp, tradeTypeTmp, open, timezone, securityType, reserve);
...
def GetPlates(includeHis = False, securityType = [], plateType = [], nodeLevel = [], plateTreeID = "", reserve = ""):
    ...
    ret = QtPyAPI.GetPlatesForPy(cols, errorMsg, includeHis, securityTypeTmp, plateTypeTmp,\
                                 nodeLevelTmp, plateTreeID, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    colsDf = pd.DataFrame(colsDict);
    return ret, errorMsg.c_str(), colsDf
...
# @param sortDefs 排序要求
# @param reserve 预留，默认为空
def GetHisMarketInfo(securities, securityIDS, fields, dateBegin, dateEnd,\
                     sortDefs = [], timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetHisMarketInfoForPy(cols, errorMsg, securitiesTmp, securityIDSTmp,\
                                        fieldsTmp, dateBegin, dateEnd, sortDefs, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
# @brief 按时间取因子数据，数据先按日期和时间排序，再按证券代码排序
# @param cols 列式结果集
# @param errorMsg 调用失败时返回错误描述信息
...
# @param reserve 预留，默认为空
def MDPGetUserFactorByTime(securities, fields, quoteType, interval, timePeriods, timezone = 8, priceAdj = EPriceAdjust["k_AdjNone"], reserve = ""):
    ...
    ret = QtPyAPI.MDPGetFactorByTimeForPy(colsSet, errorMsg, securitiesTmp, fieldsTmp, quoteType, interval,
                                      timePeriods, timezone, priceAdj, reserve);

    colsList = QtPyAPI.PraseColumnSetSeqEx(colsSet, True);
    colsDict = {};
    for iter in colsList:
        tmpDict = iter;
        if len(colsDict) == 0:
...
# @brief 按时间取因子数据，数据先按日期和时间排序，再按证券代码排序cols 列式结果集
# @param cols 列式结果集
# @param errorMsg 调用失败时返回错误描述信息
# @param securities 证券代码
# @param fields 数据字段，一个"*"表示提取全部字段
...
# @param priceAdj 复权类型，行情展示需要
# @param reserve 预留，默认为空
def MDPGetUserFactorByCount(securities, fields, quoteType, interval, dateTime, dir, count, timezone = 8, priceAdj = EPriceAdjust["k_AdjNone"], reserve = ""):
    errorMsg = QtPyAPI.string();
    colsSet = QtPyAPI.ColumnSetSeq();
    securitiesTmp = QtPyAPI.StringSeq();
    fieldsTmp = QtPyAPI.StringSeq();
    ...
    ret = QtPyAPI.MDPGetUserFactorByCount(colsSet, errorMsg, securitiesTmp, fieldsTmp, quoteType, interval,
                                          dateTime, dir, count, timezone, priceAdj, reserve);
...
def GetFactor(securities, securityIDS, fields, dateBegin, dateEnd, sortDefs = [], timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetFactorForPy(cols, errorMsg, securitiesTmp, securityIDSTmp, fieldsTmp, \
                                 dateBegin, dateEnd, sortDefs, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
# @param sortDefs 排序要求
# @param reserve 预留，默认为空
def GetFinance(securities, securityIDS, fields, dateBegin, dateEnd, rptType, trailType, dateType, \
               sortDefs = [], timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetFinanceForPy(cols, errorMsg, securitiesTmp, securityIDSTmp, fieldsTmp,\
                                  dateBegin, dateEnd, rptType, trailType, dateType, sortDefs, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
def GetSnapData(securities, securityIDS, quoteData, fields, sortDefs = [], reserve = ""):
    ...
    ret = QtPyAPI.GetSnapDataForPy(cols, errorMsg, securitiesTmp, securityIDSTmp,\
                                   quoteData, fieldsTmp, sortDefs, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
# @param timezone 参数中时间参数的时区，默认为东8区
# @param reserve 预留，默认为空
def GetSecurityCurInfo(securities, securityIDS, fields, sortDefs = [], \
                       dateBegin = "", dateEnd = "", timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetSecurityCurInfoForPy(cols, errorMsg, securitiesTmp, securityIDSTmp, \
                                          fieldsTmp, sortDefs, dateBegin, dateEnd, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
def GetSecurityInfo(securities, securityIDS, fields, sortDefs = [], updateTime = "", reserve = ""):
    ...
    ret = QtPyAPI.GetSecurityInfoForPy(cols, errorMsg, securitiesTmp, securityIDSTmp,\
                                       fieldsTmp, sortDefs, updateTime, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
def GetPlateSymbols(plateIDs, setOper, sortDefs = [], dateBegin = "", dateEnd = "", timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetPlateSymbolsForPy(cols, errorMsg, plateIDsTmp, setOper, sortDefs,\
                                       dateBegin, dateEnd, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    colsDf = pd.DataFrame(colsDict);
    return ret, errorMsg.c_str(), colsDf,
...
def GetTickByCount(security, securityID, fields, datetime, dir, count, timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetTickByCountForPy(cols, errorMsg, security, securityID, fieldsTmp, \
                                      datetime, dir, count, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
# @param sortDefs 排序要求
# @param reserve 预留，默认为空
def GetDataByCount(securities, securityIDS, fields, quoteType, interval, datetime, dir, count,\
                   sortDefs = [], timezone = 8, priceAdj = EPriceAdjust["k_AdjNone"], reserve = ""):
    ...
    ret = QtPyAPI.GetDataByCountForPy(cols, errorMsg, securitiesTmp, securityIDSTmp, fieldsTmp, \
                                      quoteType, interval, datetime, dir, count, sortDefs, timezone, priceAdj, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
# @param sortDefs 排序要求
# @param reserve 预留，默认为空
def GetDataByTime(securities, securityIDS, fields, quoteType, interval, timePeriods, \
                  sortDefs = [], timezone = 8, priceAdj = EPriceAdjust["k_AdjNone"], reserve = ""):
    errorMsg = QtPyAPI.string();
    cols = QtPyAPI.ColumnSeq();
    securitiesTmp = QtPyAPI.StringSeq();
    ...
    ret = QtPyAPI.GetDataByTimeForPy(cols, errorMsg, securitiesTmp, securityIDSTmp, fieldsTmp,\
                                     quoteType, interval, timePeriods, sortDefs, timezone, priceAdj, reserve)
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
def GetL2TickByTime(security, securityID, quoteData, fields, timePeriods, timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetL2TickByTimeForPy(cols, errorMsg, security, securityID, \
                                       quoteData, fieldsTmp, timePeriods, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
...
def GetL2TickByCount(security, securityID, quoteData, fields, datetime, dir, count, timezone = 8, reserve = ""):
    ...
    ret = QtPyAPI.GetL2TickByCountForPy(cols, errorMsg, security, securityID, quoteData, fieldsTmp,\
                                        datetime, dir, count, timezone, reserve);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
...
#@brief 查询月度数据计费清单
#@param bills 计费清单
#@param errDesc 调用失败时返回错误描述信息
#@param dateBegin 查询起始时间[dateBegin, endDate]
#@param dateEnd 截止查询时间，YYYY-MM-DD
#@param type 计费数据类型，若为BillAll则查询全部
#@param reserve 预留，默认为空
def GetMonthBill(dateBegin, dateEnd, type = EBillDataType["k_BillAll"], reserve = ""):
    bills = QtPyAPI.MonthBillSeq();
    errorMsg = QtPyAPI.string();

    ret = QtPyAPI.GetMonthBill(bills, errorMsg, dateBegin, dateEnd, type, reserve);
    nSize = bills.size();
    bills_list = []
    for iter in bills:
        bills_list.append({"month": iter.month,
                           "type": iter.type,
                           "count": iter.count,
                           "createTime": iter.createTime,
                           "updateTime": iter.updateTime,
                           "reserve": iter.reserve
                           });

    billsDf = pd.DataFrame(bills_list);
    return ret, errorMsg.c_str(), billsDf;

#@brief 查询数据计费明细
#@param bills 计费清单
#@param errDesc 调用失败时返回错误描述信息
#@param dateBegin 查询起始时间[dateBegin, endDate]
#@param dateEnd 截止查询时间，YYYY-MM-DD HH:MM:SS
#@param type 计费数据类型，若为BillAll则查询全部
#@param reserve 预留，默认为空
def GetDetailBill(dateBegin, dateEnd, type = EBillDataType["k_BillAll"], reserve = ""):
    bills = QtPyAPI.DetailBillSeq();
    errorMsg = QtPyAPI.string();

    ret = QtPyAPI.GetDetailBill(bills, errorMsg, dateBegin, dateEnd, type, reserve);

    bills_list = []
    for iter in bills:
        bills_list.append({"apiName":iter.apiName,
                           "type":iter.type,
                           "count":iter.count,
                           "createTime":iter.createTime,
                           "updateTime":iter.updateTime,
...
# @brief 财经数据库单表查询
# @param ret 返回值(输出参数)
# @param cols 列式结果集(输出参数)
# @param errorMsg 调用失败时返回错误描述信息(输出参数)
# @param tableName 查询的表名
# @param fields 查询的字段
# @param conditions 查询条件
# @param sorts 排序要求
# @param page 分页信息
def QueryTable(tableName, fields, conditions = [], sorts = [], page = []):
    errorMsg = QtPyAPI.string();
    cols = QtPyAPI.ColumnSeq();
    fieldsTmp = QtPyAPI.StringSeq();
    ...
    ret = QtPyAPI.QueryTableForPy(cols, errorMsg, tableName, fieldsTmp, conditions, sorts, page);
    colsDict = QtPyAPI.PraseColumnSeq(cols);
    if fields == ["*"]:
        colsDf = pd.DataFrame(colsDict);
    else:
